...
def save_email_fallback(subject, content):
    ...
    eml_path = fallback_dir / f"{ts}_{subject.replace(' ', '_')}.eml"
    msg = MIMEMultipart("alternative")
    msg["Subject"] = subject
...
def push_report(title, content):
    ...
    if check_smtp_reachable():
        ...
        print(f"[邮件] 网络恢复后可手动发送: python3 -c \"")
        print(f"  import smtplib; s=smtplib.SMTP_SSL('smtp.qq.com',465);")
        print(f"  s.login('{EMAIL_SENDER}','***');")
        print(f"  with open('{eml_path}') as f: s.sendmail('{EMAIL_SENDER}',{EMAIL_RECEIVERS},f.read())")
        email_ok = False
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("用法: python run_push.py <报告标题>")
        sys.exit(1)
    title = sys.argv[1]
